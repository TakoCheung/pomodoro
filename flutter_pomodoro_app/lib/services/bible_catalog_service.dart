...
class BibleCatalogService implements BibleCatalogServiceInterface {
  ...
  BibleCatalogService({http.Client? client, required this.apiKey}) : client = client ?? http.Client();
  ...
  Future<List<BibleVersion>> fetchBibles() async {
    ...
    return data.map((e) => BibleVersion.fromJson(e as Map<String, dynamic>)).toList(growable: false);
  }
}

const _prefsKeyBibleVersions = 'bible_versions_cache_v1';

final bibleCatalogServiceProvider = Provider<BibleCatalogServiceInterface>((ref) {
  final key = dotenv.env['SCRIPTURE_API_KEY'];
  if (key == null || key.isEmpty) {
    throw UnimplementedError('SCRIPTURE_API_KEY not set; cannot fetch bible catalog');
  }
...
final bibleVersionsProvider = FutureProvider<List<BibleVersion>>((ref) async {
  ...
  try {
    final enc = json.encode(fetched.map((e) => e.toJson()).toList(growable: false));

...
class FakeService implements ScriptureServiceInterface {
  ...
  Future<Passage> fetchPassage(
    ...
    return Passage(
        reference: passageId, text: 'Text for $passageId', verses: []);
  }
}
...
void main() {
  ...
  test(
      'fetchAndCacheRandomPassage appends every fetched passage to history and persists',
      () async {
    final prefs = await SharedPreferences.getInstance();
    final repo = ScriptureRepository(
        service: FakeService(), now: () => DateTime(2025, 8, 30), prefs: prefs);

    final p1 = await repo
        .fetchAndCacheRandomPassage(bibleId: 'b', passageIds: ['GEN.1.1']);
    expect(p1.reference, 'GEN.1.1');
    expect(repo.history.length, 1);
    expect(repo.history.first.reference, 'GEN.1.1');

    final p2 = await repo
        .fetchAndCacheRandomPassage(bibleId: 'b', passageIds: ['JOH.3.16']);
  ...
  test(
      'getRandomPassageOncePerDay adds to history only when fetching (first call), not when returning cached',
      () async {
    final prefs = await SharedPreferences.getInstance();
    DateTime fixedNow() => DateTime(2025, 8, 30, 10, 0, 0);
    final repo = ScriptureRepository(
        service: FakeService(), now: fixedNow, prefs: prefs);

    final p1 = await repo
        .getRandomPassageOncePerDay(bibleId: 'b', passageIds: ['PSA.23.1']);
    expect(p1.reference, 'PSA.23.1');
    expect(repo.history.length, 1);

    final p2 = await repo
        .getRandomPassageOncePerDay(bibleId: 'b', passageIds: ['PSA.23.2']);

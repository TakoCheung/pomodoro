...
void main() {
  ...
  test('fetchAndCacheRandomPassage appends every fetched passage to history and persists',
      () async {
  ...
  test(
      'selectPassageForBreak adds to history only when fetching (first call), not when returning cached',
      () async {
    final prefs = await SharedPreferences.getInstance();
    DateTime fixedNow() => DateTime(2025, 8, 30, 10, 0, 0);
    final repo = ScriptureRepository(service: FakeService(), now: fixedNow, prefs: prefs);

    final p1 = await repo.selectPassageForBreak(bibleId: 'b', passageIds: ['PSA.23.1']);
    expect(p1.reference, 'PSA.23.1');
    expect(repo.history.length, 1);

    final p2 = await repo.selectPassageForBreak(bibleId: 'b', passageIds: ['PSA.23.2']);
